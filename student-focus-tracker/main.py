...
GLOBAL_TOKEN = os.getenv("FOCUS_TOKEN")  # Global fallback token
TOKEN = None  # runtime auth token (optional)
...
def save_focus_data(tracking_data):
    """Save focus tracking data to local JSON file"""
    try:
        ...
        log_message(f"Warning: Failed to save focus data locally: {e}")
...
def run_attention_tracker(token=None, headless=False):
    ...
    # Heavy CV modules are imported lazily so startup/camera errors appear quickly.
    ...
    try:
        while cap.isOpened():
            ...
            if now - last_processed >= process_interval:
                ...
                now = time.time()
                if UPLOAD_TO_SERVER and now - last_upload >= upload_interval and focus_data_batch:
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Student Focus Tracker")
    parser.add_argument("class_id", help="Class ID to track")
    parser.add_argument("token", nargs="?", default=None, help="JWT token (optional). If omitted, will use FOCUS_TOKEN env var.")
    parser.add_argument("--headless", "--background", action="store_true", help="Run without showing a camera window")
    parser.add_argument("--no-upload", "--local-only", action="store_true", help="Disable server upload; only save locally")

    # Parse only known args so it doesn't crash if extra flags are passed by mistake.
    ...
    token = args.token or GLOBAL_TOKEN
    ...
    log_message(f"Arguments: class_id={CLASS_ID}, token={'***' if token else 'None'}, headless={args.headless}, upload={UPLOAD_TO_SERVER}")

    run_attention_tracker(token=token, headless=args.headless)
